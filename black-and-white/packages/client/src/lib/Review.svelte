<script lang="ts">
  ...
  const th = 'border-b border-white/10 px-[12px] py-[8px] text-center font-semibold tracking-[0.5px] text-gold-muted';
  const td = 'border-b border-white/[0.05] px-[12px] py-[8px] text-center text-felt-text';
...
>
  <div class="flex flex-col items-center gap-[6px]">
    {#if review.winner === 'me'}
      <span class="text-[1.8rem] font-extrabold tracking-[1px] text-win">你赢了 🎉</span>
    {:else if review.winner === 'opp'}
      <span class="text-[1.8rem] font-extrabold tracking-[1px] text-lose">你输了</span>
    {:else}
      <span class="text-[1.8rem] font-extrabold tracking-[1px] text-gold-muted">平局</span>
    {/if}
