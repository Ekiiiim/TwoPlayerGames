...
>
  <h1 class="m-0 text-[2.2rem] font-extrabold tracking-[4px] text-gold">黑与白</h1>
  <p class="text-[0.85rem] tracking-[1px] text-gold-muted">《游戏的法则IV》Ep.01</p>

  <div class="h-px w-full bg-white/[0.08]"></div>

  <div class="flex w-full flex-col items-center gap-[14px]">
    {#if $roomCode}
      <Button variant="ghost" class="w-full" on:click={leaveRoom}>解散房间</Button>
      <div
      ...
      >
        <span class="text-[0.75rem] uppercase tracking-[1px] text-gold-muted">房间码</span>
        <strong class="text-[2rem] font-extrabold tracking-[6px] text-gold">{$roomCode}</strong>
        <span class="mt-[2px] text-[0.78rem] text-gold-muted">发给朋友，等待对手加入…</span>
      </div>
    {:else}
