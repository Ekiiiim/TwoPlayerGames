      ...
      {#if cell}
        <div
          class="flex h-12 w-12 items-center justify-center rounded-[10px] border-2 text-2xl font-bold transition
                 {showsBack(cell.index) ? 'border-accent bg-back text-accent' : 'border-accent bg-card text-ink'}"
        >
          {showsBack(cell.index) ? backText(cell) : cell.letter}
        </div>
      {:else}
        <div class="h-12 w-12 rounded-[10px] border-2 border-dashed border-line"></div>
      {/if}
    {/each}
  </div>
</div>
